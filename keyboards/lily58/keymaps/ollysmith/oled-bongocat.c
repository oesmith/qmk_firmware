...
#else
	is_keyboard_left() ? decode_frame(idle_anim[current_frame]) : decode_frame(left_idle_anim[current_frame]);
#endif
}
...
static void render_cat_prep(void) {
	...
	decode_frame(prep);
#else
	is_keyboard_left() ? decode_frame(prep) : decode_frame(left_prep);
#endif
}
...
#else
	is_keyboard_left() ? decode_frame(tap_anim[current_frame]) : decode_frame(left_tap_anim[current_frame]);
#endif
}
